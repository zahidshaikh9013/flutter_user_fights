...
class AgeCalculator {
  ...
  static DateDuration dateDifference({
    ...
    if (fromDate.month > endDate.month) {
      ...
      if (fromDate.day > endDate.day) {
        months--;
        days = daysInMonth(fromDate.year + years, ((fromDate.month + months - 1) % DateTime.monthsPerYear) + 1) +
            endDate.day -
            fromDate.day;
      } else {
  ...
  /// add method
  static DateTime subtract({required DateTime date, required DateDuration duration}) {
    int years = date.year - duration.years;
    years -= ((date.month + duration.months) ~/ DateTime.monthsPerYear);
    int months = ((date.month + duration.months) % DateTime.monthsPerYear);
  ...
  static DateDuration timeToNextBirthday(DateTime birthdate, {DateTime? fromDate}) {
    ...
    DateTime nextBirthdayDate =
        tempDate.isBefore(endDate) ? AgeCalculator.add(date: tempDate, duration: DateDuration(years: 1)) : tempDate;
